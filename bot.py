...
import aiosqlite  # For async DB operations where needed, but keeping sync for simplicity

# Load environment variables
DISCORD_TOKEN = os.getenv('DISCORD_TOKEN', 'MTUxNjUyMTYyOTM5ODcyODc4NQ.GazHg4.pexUZCC6ofI3L82VkqHhKS-5wPFc4wtMeO8Quw')
BOT_NAME = os.getenv('BOT_NAME', 'ShadowNetwork')
PREFIX = os.getenv('PREFIX', '!')
YOUR_SERVER_IP = os.getenv('YOUR_SERVER_IP', '127.0.0.1')
MAIN_ADMIN_ID = int(os.getenv('MAIN_ADMIN_ID', '1391705452240437290'))
VPS_USER_ROLE_ID = int(os.getenv('VPS_USER_ROLE_ID', '1517281041449226302'))
...
# Database setup with improved error handling and transactions
DB_LOCK = threading.Lock()  # For thread safety on DB ops

@contextmanager
def get_db_connection():
    conn = None
    try:
        conn = sqlite3.connect('vps.db', check_same_thread=False)
        conn.row_factory = sqlite3.Row
        conn.execute("PRAGMA journal_mode=WAL")  # Better concurrency
        conn.execute("PRAGMA synchronous=NORMAL")  # Balance speed/safety
        conn.execute("PRAGMA cache_size=10000")  # Improve performance
        yield conn
...
def init_db():
    with get_db_connection() as conn:
        ...
        cur.execute('''CREATE TABLE IF NOT EXISTS port_forwards (
            ...
            created_at TEXT NOT NULL,
            FOREIGN KEY (user_id) REFERENCES port_allocations (user_id)
        )''')

        # Add indexes for performance
        cur.execute('CREATE INDEX IF NOT EXISTS idx_vps_user ON vps(user_id)')
        cur.execute('CREATE INDEX IF NOT EXISTS idx_vps_container ON vps(container_name)')
        cur.execute('CREATE INDEX IF NOT EXISTS idx_port_forwards_user ON port_forwards(user_id)')
        cur.execute('CREATE INDEX IF NOT EXISTS idx_port_forwards_container ON port_forwards(vps_container)')

        # Vacuum to prevent bloat
        conn.commit()

        try:
            vacuum_conn = sqlite3.connect('vps.db')
...
def get_vps_data() -> Dict[str, List[Dict[str, Any]]]:
    ...
    with get_db_connection() as conn:
        ...
        for row in rows:
            ...
            vps = dict(row)
            try:
                vps['shared_with'] = json.loads(vps['shared_with'] or '[]')
                vps['suspension_history'] = json.loads(vps['suspension_history'] or '[]')
            except json.JSONDecodeError as e:
                logger.error(f"JSON decode error for VPS {vps['id']}: {e}. Using defaults.")
                vps['shared_with'] = []
...
# Port forwarding functions with better error handling
...
def get_available_host_port() -> Optional[int]:
    ...
    for _ in range(100):
        ...
        if port not in used_ports:
            return port
    logger.warning("Could not find available host port after 100 attempts.")
    return None
...
async def create_port_forward(user_id: str, container: str, vps_port: int) -> Optional[int]:
    ...
    try:
        ...
        with get_db_connection() as conn:
            ...
            conn.commit()
        logger.info(f"Created port forward: {host_port} -> {container}:{vps_port}")
        return host_port
    except Exception as e:
        logger.error(f"Failed to create port forward: {e}")
        # Cleanup if partial failure
        try:
            await execute_lxc(f"lxc config device remove {container} tcp_proxy_{host_port}", timeout=10)
            await execute_lxc(f"lxc config device remove {container} udp_proxy_{host_port}", timeout=10)
        except:
            pass
        return None
...
async def remove_port_forward(forward_id: int, is_admin: bool = False) -> tuple[bool, Optional[str]]:
    ...
    try:
        ...
        logger.info(f"Removed port forward {forward_id}")
        return True, user_id
    except Exception as e:
        logger.error(f"Failed to remove port forward devices {forward_id}: {e}")
        # Rollback DB if device removal fails
        with get_db_connection() as conn:
            cur = conn.cursor()
            cur.execute('INSERT INTO port_forwards (user_id, vps_container, vps_port, host_port, created_at) VALUES (?, ?, ?, ?, ?)',
                        (user_id, container, 0, host_port, datetime.now().isoformat()))  # Placeholder to revert
            conn.commit()
        return False, None
...
# Initialize database
# finish all DB creation and updates
init_db()

vps_data = get_vps_data()
admin_data = {'admins': get_admins()}

# now run VACUUM safely using a new connection
try:
    vac = sqlite3.connect("vps.db")
    vac.execute("VACUUM;")
    vac.close()
except Exception as e:
    logger.warning(f"VACUUM skipped: {e}")
...
def create_embed(title, description="", color=0x1a1a1a):
    ...
    embed.set_thumbnail(url="https://i.ibb.co/fd0CC0bt/sn-shadownetwork.jpg")
    embed.set_footer(text=f"{BOT_NAME} VPS Manager • {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}",
                     icon_url="https://i.ibb.co/fd0CC0bt/sn-shadownetwork.jpg")
    return embed
...
async def execute_lxc(command, timeout=120):
    try:
        ...
        if proc.returncode != 0:
            ...
            raise Exception(f"Return code {proc.returncode}: {error}")
    ...
    try:
        ...
        try:
            ...
            if "already exists" not in str(e).lower():
                logger.warning(f"Fuse device add failed but continuing: {e}")
...
async def apply_internal_permissions(container_name):
    try:
        ...
        for cmd in commands:
            try:
                await execute_lxc(f"lxc exec {container_name} -- bash -c \"{cmd}\"", timeout=30)
...
# Helper functions for container stats with improved error handling and stopped state awareness
async def get_container_status(container_name):
    try:
        result = await execute_lxc(f"lxc info {container_name}", timeout=10)
        for line in result.splitlines() if isinstance(result, str) else []:
...
async def get_container_cpu_pct(container_name):
    try:
        result = await execute_lxc(f"lxc exec {container_name} -- top -bn1", timeout=15)
...
async def get_container_memory(container_name):
    ...
    if status != 'running':
        return "N/A (Stopped)"
    try:
        result = await execute_lxc(f"lxc exec {container_name} -- free -m", timeout=10)
...
async def get_container_disk(container_name):
    ...
    if status != 'running':
        return "N/A (Stopped)"
    try:
        result = await execute_lxc(f"lxc exec {container_name} -- df -h /", timeout=10)
        lines = result.splitlines()
        for line in lines:
...
async def get_container_uptime(container_name):
    ...
    if status != 'running':
        return "N/A (Stopped)"
    try:
        result = await execute_lxc(f"lxc exec {container_name} -- uptime", timeout=10)
...
class ManageView(discord.ui.View):
    ...
    async def action_callback(self, interaction: discord.Interaction, action: str):
        if str(interaction.user.id) != self.user_id and not self.is_admin:
            await interaction.response.send_message(embed=create_error_embed("Access Denied", "This is not your Phoenix VPS!"), ephemeral=True)
            return
        ...
        if action == 'reinstall':
            if self.is_shared or self.is_admin:
                await interaction.response.send_message(embed=create_error_embed("Access Denied", "Only the Phoenix VPS owner can reinstall!"), ephemeral=True)
                return
            if suspended:
                await interaction.response.send_message(embed=create_error_embed("Cannot Reinstall", "Unsuspend the Phoenix VPS first."), ephemeral=True)
                return
            ...
            confirm_embed = create_warning_embed("Reinstall Warning",
                f"⚠️ **WARNING:** This will erase all data on Phoenix VPS `{container_name}` and reinstall a fresh OS.\n\n"
...
async def vps_info(ctx, container_name: str = None):
    if not container_name:
        ...
        embed = create_embed(f"🖥️ Phoenix VPS Information - {container_name}", f"Details for VPS owned by {found_user.mention}{suspended_text}{whitelisted_text}", 0x1a1a1a)
...
async def restart_vps(ctx, container_name: str):
    await ctx.send(embed=create_info_embed("Restarting Phoenix VPS", f"Restarting VPS `{container_name}`..."))
    try:
    ...
    for uid, lst in vps_data.items():
        for vps in lst:
            if vps['container_name'] == container_name:
                ...
                try:
                    owner = await bot.fetch_user(int(uid))
                    embed = create_success_embed("🟢 VPS Unsuspended", f"Your Phoenix VPS `{container_name}` has been unsuspended by an admin.\nYou can now manage it again.")
                    await owner.send(embed=embed)
                except Exception as dm_e:
...
class HelpView(discord.ui.View):
    ...
    def update_embed(self):
        ...
        footers = {
            "user": f"{BOT_NAME} Phoenix VPS Manager • User Commands",
            "vps": f"{BOT_NAME} Phoenix VPS Manager • VPS Management • Snapshots • Cloning",
            "ports": f"{BOT_NAME} Phoenix VPS Manager • Port Forwarding • TCP/UDP Support",
            "system": f"{BOT_NAME} Phoenix VPS Manager • System Monitoring • Resource Management",
            "bot": f"{BOT_NAME} Phoenix VPS Manager • Bot Control • Status Management",
            "admin": f"{BOT_NAME} Phoenix VPS Manager • Admin Panel • Restricted Access",
            "main_admin": f"{BOT_NAME} Phoenix VPS Manager • Main Admin • Full System Control"
        }
  
        self.embed.set_footer(text=footers.get(self.current_category, f"{BOT_NAME} Phoenix VPS Manager"))
